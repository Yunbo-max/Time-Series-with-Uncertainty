...
class NeuralHarmonicModel(torch.nn.Module):
    ...
    def forward(self, x, t, trend=None):
        ...
        # return yhat_season + yhat_trend

        # return fourier_components + yhat_trend
        # return yhat_season + yhat_trend
...
y_pred_tensor_all = nn_model(x=X_tensor, t=t_tensor, trend=trend_tensor)

plt.figure()
plt.plot(Y_all, label="TRUE")
plt.plot(y_pred_tensor_all.detach().cpu().numpy().flatten(), label="PRED")
plt.legend()

print(f"MAE: {(torch.abs(y_pred_tensor_all - Y_tensor)).mean().item()}")
print(f"R2 : {r2_score(Y_all, y_pred_tensor_all.detach().cpu().numpy())}")
